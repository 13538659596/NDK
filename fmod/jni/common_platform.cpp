...
int gDownButtons;        //按钮按下时赋值，按钮抬起时清零
...
{
	LOGE("------->  gPressedButtons %d\n", gPressedButtons);
	LOGE("------->%d\n", (gPressedButtons & (1 << btn)) != 0);
...
jstring Java_com_example_fmod_MainActivity_getButtonLabel(JNIEnv *env, jobject thiz, jint index)
{
    return env->NewStringUTF(Common_BtnStr((Common_Button)index));
}

void Java_com_example_fmod_MainActivity_buttonDown(JNIEnv *env, jobject thiz, jint index)
{
    gDownButtons |= (1 << index);
}

void Java_com_example_fmod_MainActivity_buttonUp(JNIEnv *env, jobject thiz, jint index)
{
	//手指抬起时  将 gDownButtons 值清零
    gDownButtons &= ~(1 << index);
}

void Java_com_example_fmod_MainActivity_setStateCreate(JNIEnv *env, jobject thiz)
{

}

void Java_com_example_fmod_MainActivity_setStateStart(JNIEnv *env, jobject thiz)
{
	gSuspendState = false;
}

void Java_com_example_fmod_MainActivity_setStateStop(JNIEnv *env, jobject thiz)
{
	gSuspendState = true;
}

void Java_com_example_fmod_MainActivity_setStateDestroy(JNIEnv *env, jobject thiz)
{
	gQuitState = true;
}

void Java_com_example_fmod_MainActivity_main(JNIEnv *env, jobject thiz)
{
	gJNIEnv = env;
